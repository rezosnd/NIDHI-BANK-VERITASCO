...
const Calendar = ({ size = 24 }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>;
...
const ApproveTransactions = () => {
  ...
  const fetchTransactions = async () => {
    ...
    // Format date as dd/MM/yyyy as typically stored
    ...
    try {
      const res = await fetch('/api/transactions/pending', {
        ...
        body: JSON.stringify({ date: formattedDate }) // Send exact match format
      });
      ...
      if (data.success === false) {
        ...
        if (data.length === 0) setMessage('No pending transactions found for this date.');
      }
    } catch (err) {
  ...
  const handleStatusUpdate = async (status) => {
    if (selectedIds.length === 0) {
      setMessage(`Please select at least one transaction to ${status.toLowerCase()}.`);
      return;
    }
    ...
    try {
      ...
      setMessage(`Error attempting to ${status.toLowerCase()} transactions.`);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="solid-panel" style={{ padding: '24px', marginBottom: '24px' }}>
        ...
        {/* Filter Bar */}
        <div className="filter-panel" style={{ padding: '20px', marginBottom: '24px', display: 'flex', justifyContent: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '16px' }}>
            <div style={{ width: '250px' }}>
              <label className="solid-label">Date :</label>
              <input 
                type="date" 
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead>
              <tr className="table-solid-header" style={{ textAlign: 'left' }}>
                ...
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Member ID</th>
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Name</th>
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Transaction Type</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={8} style={{ padding: '32px', textAlign: 'center', color: '#475569' }}>
                    <div style={{ fontWeight: '600' }}>Loading pending transactions...</div>
                  </td>
                </tr>
              ) : transactions.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: '48px', textAlign: 'center', color: '#64748b' }}>
                    ...
                    <div style={{ fontSize: '0.9rem', fontWeight: '600', color: '#334155' }}>No Pending Transactions Available</div>
                  </td>
                </tr>
              ) : (
                transactions.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="table-row-hover" style={{ borderBottom: '1px solid rgba(255,255,255,0.4)', backgroundColor: isChecked ? 'rgba(239, 246, 255, 0.5)' : 'transparent' }}>
                      ...
                      <td style={{ padding: '12px', color: '#475569', fontWeight: '600', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
                        {t.type || '-'}
                      </td>
                      <td style={{ padding: '12px', color: '#166534', fontWeight: '800', textAlign: 'right', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
                        {parseFloat(t.amount).toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}
                      </td>
