...
const CustomerFeedback = () => {
  ...
  const fetchReport = async () => {
    ...
    setMessage('');
    try {
      const res = await fetch('/api/customer-feedback/search', {
        method: 'POST',
        headers: { 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.4)', paddingBottom: '16px', marginBottom: '20px' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.25rem', color: '#0f172a', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0052cc" strokeWidth="2.5"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>
              Customer Feedback / Appreciation Report
            </h2>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead>
              <tr className="table-solid-header" style={{ textAlign: 'left' }}>
                ...
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Feedback</th>
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Rating</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} style={{ padding: '32px', textAlign: 'center', color: '#475569' }}>
                ...
                feedbacks.map((f) => (
                  <tr key={f.id} className="table-row-hover" style={{ borderBottom: '1px solid rgba(255,255,255,0.4)' }}>
                    <td style={{ padding: '12px', color: '#334155', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
                      {new Date(f.issue_date).toLocaleDateString()}
                    </td>
                    ...
                    <td style={{ padding: '12px', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
                      <div style={{ fontWeight: 'bold', color: '#0f172a' }}>{f.name}</div>
                      <div style={{ fontSize: '11px', color: '#475569' }}>{f.mobile}</div>
                      <div style={{ fontSize: '11px', color: '#475569' }}>{f.email}</div>
                    </td>
                    <td style={{ padding: '12px', color: '#0f172a', fontWeight: '600', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
                      {f.service_type || 'General'}
                    </td>
                    <td style={{ padding: '12px', color: '#334155', borderRight: '1px solid rgba(255,255,255,0.4)', maxWidth: '250px' }}>
                      {f.feedback_text || '-'}
                    </td>
                    <td style={{ padding: '12px', color: '#eab308', letterSpacing: '2px', fontSize: '1.2rem', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
