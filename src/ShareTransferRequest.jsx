...
const ShareTransferRequest = () => {
  ...
  useEffect(() => {
    // Fetch branches and check user role
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch('/api/share-transfers/search', {
        method: 'POST',
        headers: { 
      ...
      if (data.success === false) {
        ...
        setRequests(data);
        if (data.length === 0) setMessage('No share transfer requests found for the selected criteria.');
      }
    } catch (err) {
      setMessage('Error fetching share transfer requests.');
    }
    setLoading(false);
  };
  ...
  const handleStatusUpdate = async (status) => {
    if (selectedIds.length === 0) {
      setMessage(`Please select at least one request to ${status.toLowerCase()}.`);
      return;
    }
    ...
    try {
      const res = await fetch('/api/share-transfers/status', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      setMessage(`Error attempting to ${status.toLowerCase()} requests.`);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="solid-panel" style={{ padding: '24px', marginBottom: '24px' }}>
        ...
        <div className="filter-panel" style={{ padding: '20px', marginBottom: '24px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'flex-end' }}>
            ...
            <div style={{ flex: '2 1 250px' }}>
              <label className="solid-label">Search Member ID / Name :</label>
              <input 
                type="text" 
                className="solid-input"
                placeholder="Search Transferor or Transferee..."
                value={searchVal}
        ...
        {!isBranchUser && selectedIds.length > 0 && (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>
            <thead>
              <tr className="table-solid-header" style={{ textAlign: 'left' }}>
                ...
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Transferor ID</th>
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Transferor Name</th>
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Transferee ID</th>
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)' }}>Transferee Name</th>
                <th style={{ padding: '12px', fontWeight: '600', border: '1px solid rgba(255, 255, 255, 0.1)', textAlign: 'center' }}>Shares</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={!isBranchUser ? 10 : 9} style={{ padding: '32px', textAlign: 'center', color: '#475569' }}>
                    <div style={{ fontWeight: '600' }}>Loading share transfer requests...</div>
                  </td>
                </tr>
              ) : requests.length === 0 ? (
                <tr>
                  <td colSpan={!isBranchUser ? 10 : 9} style={{ padding: '48px', textAlign: 'center', color: '#64748b' }}>
                ...
                requests.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="table-row-hover" style={{ borderBottom: '1px solid rgba(255,255,255,0.4)', backgroundColor: isChecked ? 'rgba(239, 246, 255, 0.5)' : 'transparent' }}>
                      ...
                      <td style={{ padding: '12px', color: '#334155', fontWeight: '600', textAlign: 'center', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
                        {t.no_of_shares || 0}
                      </td>
                      <td style={{ padding: '12px', color: '#166534', fontWeight: '800', textAlign: 'right', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
                        {parseFloat(t.transfer_amount || 0).toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}
                      </td>
                      <td style={{ padding: '12px', textAlign: 'center', borderRight: '1px solid rgba(255,255,255,0.4)' }}>
...
export default ShareTransferRequest;
